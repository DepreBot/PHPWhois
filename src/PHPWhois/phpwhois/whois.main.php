...
class PWhois extends WhoisClient
{
	// Deep whois ?
	var $deep_whois = true;
	 ...
	 *  Lookup query
	 * 
	 * @param 	string 		domain to get whois for
	 * @param 	boolean		is this a non-english domain?
	 *
	...
	function Lookup($query = '', $is_utf = true)
	{
		...
		// If query is an ip4 address do ip4 lookup
		...
		// If query is an ip6 address do ip6 lookup
		if (strpos($query, ':'))
		{
		...
		if ($tld && $server)
		{
			...
			foreach($tldtests as $htld)
			{
				...
				// if (($fp = @fopen('whois.'.$htld.'.php', 'r', 1)) and fclose($fp))   //  #oldcode
